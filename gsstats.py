...
# Default Google API key location is /home/gamer/api/playkey.json
...
def time_difference(time0, time1):
    seconds = (time.mktime(time.strptime(time1, "%Y-%m-%d %H:%M:%S")) - time.mktime(time.strptime(time0,
               "%Y-%m-%d %H:%M:%S")))
...
def get_steamdb():
    steam_api_url = 'https://api.steampowered.com/ISteamApps/GetAppList/v0001/'
    with urllib.request.urlopen(steam_api_url) as url:
...
def get_game_info(appid):
    store_api_url = "http://store.steampowered.com/api/appdetails?appids={}&filters=basic".format(appid)
    with urllib.request.urlopen(store_api_url) as url:
        data = json.loads(url.read().decode())
...
def get_sessions(data):
    ...
    for i, value in enumerate(data):
        try:
            ...
            # Game name
            elif '<GAME_CODE>' in value:
                result[session_id]["Game"] = value.split('<GAME_CODE>')[-1].split('<')[0]
...
def main(args):
    ...
    for vm in vms:
        ...
        if "yesterday" in s.keys():
            ...
            y_log = get_log(vm, (now_time-timedelta(days=td+1)).strftime('%Y-%m-%d 00:00:00'),
                            (now_time-timedelta(days=td+1)).strftime('%Y-%m-%d 23:59:59'),
                            reverse=True, debug=args.debug, logdir=logdir)
            for value in y_log:
                if ' CreateSession: session_id' in value:
                    y_session_start_time = value.split(' ')[0][:-5].replace('T', ' ')
                    break
            y_log = get_log(vm, y_session_start_time,
                            (now_time-timedelta(days=td+1)).strftime('%Y-%m-%d 23:59:59'), logdir=logdir)
        ...
        for k, v in s.items():
            ...
            if "End Time" not in v.keys():
                s.pop(k)
            else:
                s[k]["Duration"] = time_difference(v["Start Time"], v["End Time"])
        sessions.update(s)

    df_cols = ['Start Time', 'End Time', 'Host', 'VM', 'Game', 'ID', 'Continent', 'Country', 'Region', 'City',
               'Latitude',
...
if __name__ == "__main__":
    ...
    group.add_argument('--day', type=int, action='store', dest='day',
                       help='Day to analyze for sessions. 0 - today, 1 - yesterday etc.')
    ...
    parser.add_argument('--key', type=str, action='store', dest="key_path", default="/opt/playkey/gsstats/keys/playkey.json",
                        help='Path to Google API key')
